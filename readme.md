
# Copilot Studio Essentials

## Overview

- [Agent of Changes](https://www.microsoft.com/en-us/worklab/agents-of-change)


## Exercises

- [Part 1: Create your AI Agent](exercises/create-your-ai-agent/README.md)
- [Part 2: Publish my agent to Live!](exercises/deploy-my-agent-to-live/README.md)
- [Part 3: Adding Knowledge to Agent](exercises/adding-knowledge-to-agent/README.md)
- [Part 4: Define Agent Task with Rule-based flow](exercises/define-agent-task-with-rule-based-flow/README.md)

## Optional

1. [Export Agent to another Environment](exercises/export-agent-to-another-environment/README.md)
